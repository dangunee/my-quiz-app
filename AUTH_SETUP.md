...
(선택) `NEXT_PUBLIC_SITE_URL`: 배포 URL (예: `https://apps.mirinae.jp`). 이메일 확인 리다이렉트용.
...
3. **Authentication** → **URL Configuration** (중요!)
   - **Site URL**: `https://apps.mirinae.jp` (실제 배포 URL로 설정)
   - **Redirect URLs**에 추가: `https://apps.mirinae.jp/login`, `https://apps.mirinae.jp/**`
